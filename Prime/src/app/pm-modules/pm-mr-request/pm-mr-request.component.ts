...
export class PmMrRequestComponent implements OnInit {
  ...
  submitRequest() {
    ...
    for (var i = 0; i < mrList?.value.length; i++) {
      this.mrForm.value.MR_REQ1[i].CONTAINER_NO = this.mrDetailsList[0].CONTAINER_NO;
      ...
      this.mrForm.value.MR_REQ1[i].DEPO_CODE = localStorage.getItem("usercode");
      this.mrForm.value.MR_REQ1[i].CREATED_BY = localStorage.getItem("username");
    }
