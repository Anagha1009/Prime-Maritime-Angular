...
export class RenewPwdComponent implements OnInit {
  ...
  resetPwd() {
    ...
    if (
      ...
      this.renewPwd.confirmPassword=this.changePwdForm.get('CONFIRM_PWD').value;

      console.log(JSON.stringify(this.renewPwd));

      this._loginservice.renewPwd(this.renewPwd.email,this.renewPwd.emailToken,this.renewPwd.newPassword,this.renewPwd.confirmPassword)
      .subscribe((res: any) => {
        if(res.responseCode==200){
          ...
          this._commonService.errorMsg("Password reset Failed!")
        }
