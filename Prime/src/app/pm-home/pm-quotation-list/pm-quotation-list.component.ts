...
export class PmQuotationListComponent implements OnInit {
  ...
  getDetails(SRR_NO: string) {
    this._router.navigateByUrl('/pm/srr-details/' + SRR_NO);
  }
  ...
  postSelectedRateRequest(item: any, event: any) {
    ...
    if (event.target.checked) {
      ...
      this.showButtons = true;
      this.selectedQuotations = this.selectedQuotations.filter(
        (ele) => ele.SRR_NO !== item.SRR_NO
      );
    }
  ...
  approveQuotations() {
    ...
    this._quotationService
      .updateSRR(this.selectedQuotations)
      .subscribe((res: any) => {
        if (res.responseCode == 200) {
          this._commonService.successMsg(
            'Quotations are approved succesfully!'
          );
          this.getQuotationList();
        }
      });
  ...
  rejectQuotations() {
    ...
    this._quotationService
      ...
      .subscribe((res: any) => {
        if (res.ResponseCode == 200) {
          this._commonService.successMsg(
            'Quotations are Rejected succesfully!'
          );
          this.getQuotationList();
        }
      });
